...
CARTAS_PATH = BASE_DIR / "data" / "cartas.json"


with open(CARTAS_PATH, "r", encoding="utf-8") as arquivo:
...
def sortear_carta():
    ...
    disponiveis = [carta for carta in cartas if carta not in state.cartas_usadas]

    if not disponiveis:

        state.cartas_usadas = []

        disponiveis = cartas.copy()

    carta = random.choice(disponiveis)

    state.cartas_usadas.append(carta)
...
async def executar_cronometro(enviar_estado_callback):

    try:

        while (
            state.jogo_iniciado
            and not state.jogo_finalizado
            and state.carta_revelada
            and not state.turno_travado
            and not state.partida_pausada
...
def iniciar_timer(enviar_estado_callback):
    ...
    if (
        ...
        state.timer_task = asyncio.create_task(
            executar_cronometro(enviar_estado_callback)
        )
...
def pausar_turno_por_desconexao(player_id):

    if (
        state.jogo_iniciado
        and not state.jogo_finalizado
        and state.jogador_atual_id == player_id
        and not state.turno_travado
    ):
        ...
        state.jogador_pausado_id = player_id
...
def verificar_pausa():
    ...
    if (
        state.jogo_iniciado
        and not state.jogo_finalizado
        and state.jogador_atual_id
        and not jogadores_core.jogador_conectado(state.jogador_atual_id)
        and not state.turno_travado
    ):
...
def retomar_jogador(player_id, enviar_estado_callback):

    if (
        state.jogo_iniciado
        and not state.jogo_finalizado
        and state.jogador_atual_id == player_id
        and state.partida_pausada
        and state.jogador_pausado_id == player_id
    ):
        ...
        if state.carta_revelada:
            ...
            iniciar_timer(enviar_estado_callback)
...
def preparar_turno():
    ...
    state.carta_atual = sortear_carta()
