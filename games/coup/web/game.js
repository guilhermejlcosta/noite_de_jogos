...
const TOKEN_KEY = "noiteDeJogosToken",
    ...
    mostrar = (id, v) => el(id).classList.toggle("hidden", !v),
    enviar = d => socket?.readyState === WebSocket.OPEN && socket.send(JSON.stringify(d));

function conectar() {
    if (voltando || (socket && socket.readyState <= 1)) return;
    el("status").textContent = "Conectando...";
    const p = location.protocol === "https:" ? "wss" : "ws";
    socket = new WebSocket(`${p}://${location.host}/ws/jogos/coup`);
    socket.onopen = () => {
    ...
    socket.onerror = () => {
        el("status").textContent = "Não foi possível conectar ao jogo."
    };
    socket.onclose = () => {
        socket = null;
...
function processar(d) {
    ...
    if (d.tipo === "sessao_invalida") {
        ...
        mostrar("entrada", true);
        el("status").textContent = "Sessão não encontrada. Use seu código de recuperação.";
        return
    }
...
function alvoSelect(d) {
    const s = document.createElement("select");
    s.id = "alvo";
    d.jogadores.filter(j => j.id !== d.meu_id && j.influencias > 0).forEach(j => {
...
function acao(d, tipo, nome, alvo = false, classe = "") {
    const b = botao(nome, () => enviar({
        acao: "agir",
        tipo,
        alvo: alvo ? el("alvo").value : null
    }), classe);
    el("acoes").append(b)
}
...
function montarAcoes(d, eu) {
    ...
    if ((d.fase === "reacao_acao" || d.fase === "reacao_alvo") && d.meu_id === pendente.ator) {
        el("reacoes").append(botao("Continuar ação", () => enviar({
            acao: "continuar"
        })));
    }
    ...
    if (d.fase === "reacao_alvo" && d.meu_id !== pendente.ator && eu.influencias) {
        if (pendente.tipo === "ajuda") {
            el("reacoes").append(botao("Bloquear com Duque", () => enviar({
                acao: "bloquear",
                papel: "Duque"
            })));
        }
        if (pendente.alvo === d.meu_id && pendente.tipo === "roubar") {
            el("reacoes").append(
                botao("Bloquear: Capitão", () => enviar({
                    acao: "bloquear",
                    papel: "Capitão"
                })),
                botao("Bloquear: Embaixador", () => enviar({
                    acao: "bloquear",
                    papel: "Embaixador"
                }))
            );
        ...
        if (pendente.alvo === d.meu_id && pendente.tipo === "assassinar") {
            el("reacoes").append(botao("Bloquear com Condessa", () => enviar({
                acao: "bloquear",
                papel: "Condessa"
            })));
        }
    ...
    if (d.fase === "reacao_bloqueio" && d.meu_id === pendente.ator) {
        el("reacoes").append(
            botao("Aceitar bloqueio", () => enviar({
                acao: "aceitar_bloqueio"
            })),
            botao(`Desafiar ${pendente.papel_bloqueio}`, () => enviar({
                acao: "desafiar_bloqueio"
            }), "danger")
        );
    }
}

function linhaJogador(j, d) {
    const div = document.createElement("div");
    div.className = `linha${j.id===d.jogador_atual_id?" atual":""}${j.influencias?"":" eliminado"}${j.conectado?"":" offline"}`;
    const info = document.createElement("div");
    info.className = "jogador-info";
    const n = document.createElement("strong");
    n.textContent = `${j.nome}${j.host?" · HOST":""}`;
    const c = document.createElement("small");
    c.textContent = `${j.moedas} moeda(s)`;
    info.append(n, c);
    const inf = document.createElement("span");
    inf.className = "influencias";
    inf.textContent = "●".repeat(j.influencias) + "○".repeat(2 - j.influencias);
    div.append(info, inf);
    return div
}

function render(d) {
    const eu = d.jogadores.find(j => j.id === d.meu_id);
    if (!eu) return;
    ...
    mostrar("comecar", d.sou_host);
    mostrar("aguarde", !d.sou_host);
    el("jogadoresSala").replaceChildren(...d.jogadores.map(j => linhaJogador(j, d)));
    if (d.jogo_iniciado) {
        const atual = d.jogadores.find(j => j.id === d.jogador_atual_id);
        el("vez").textContent = atual ? `Vez de ${atual.nome}` : "";
        el("fase").textContent = d.fase.replaceAll("_", " ");
        el("mensagem").textContent = d.mensagem;
        el("minhasCartas").replaceChildren(...eu.cartas.map(c => {
            const x = document.createElement("div");
            x.className = `carta${c.viva?"":" perdida"}`;
            x.textContent = c.papel || "Influência";
            return x
        }));
        el("jogadores").replaceChildren(...d.jogadores.map(j => linhaJogador(j, d)));
        el("historico").replaceChildren(...d.historico.map(t => {
    ...
    if (d.jogo_finalizado) {
        el("vencedor").textContent = d.vencedor ? `🏆 ${d.vencedor} venceu!` : "Partida encerrada";
