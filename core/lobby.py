...
REGRAS_JOGOS = {
    "nao_pode": {
        ...
        "como_jogar": [
            "O jogador da vez revela sua carta.",
            "Ele explica a palavra durante o tempo da rodada.",
            "O HOST marca acerto ou erro e o turno avança.",
        ],
    ...
    "mimica": {
        ...
        "como_jogar": [
            "O jogador da vez vê o tema secreto.",
            "Ele faz a mímica sem falar, escrever ou apontar letras.",
            "O HOST registra o resultado antes do tempo terminar.",
        ],
    ...
    "quem_sou_eu": {
        ...
        "como_jogar": [
            "Cada pessoa vê as identidades dos outros, mas não a própria.",
            "Faça perguntas que possam ser respondidas com sim ou não.",
            "Use as respostas para descobrir quem ou o que você é.",
        ],
    ...
    "mais_provavel": {
        ...
        "como_jogar": [
            "Leia a situação da rodada.",
            "Cada pessoa vota secretamente em um jogador.",
            "Os votos são revelados juntos e acumulados até o resultado final.",
        ],
    ...
    "stop": {
        ...
        "como_jogar": [
            "Uma letra é sorteada para todos.",
            "Preencha uma resposta válida em cada categoria.",
            "Resposta única vale 10, repetida vale 5 e inválida vale 0 pontos.",
        ],
    ...
    "quiz": {
        ...
        "como_jogar": [
            "Uma pergunta e suas alternativas aparecem na tela.",
            "Cada jogador escolhe sua resposta.",
            "O jogo revela a correta, contabiliza os pontos e avança.",
        ],
    ...
    "ito": {
        ...
        "como_jogar": [
            ...
            "Dê uma pista adequada ao tema e à posição do seu número na escala.",
            "O HOST organiza as pistas; depois os números são revelados e o jogo calcula os erros.",
        ],
    ...
    "coup": {
        ...
        "como_jogar": [
            "Use ações de personagens, mesmo sem possuir a carta: blefar é permitido.",
            "Outros podem desafiar sua declaração ou bloquear determinadas ações.",
            "Quem perde um desafio ou sofre um ataque perde influência; com 10 moedas, o Golpe é obrigatório.",
        ],
...
async def enviar_estado_lobby():
    ...
    for viewer_id, viewer in list(jogadores_core.jogadores.items()):

        websocket = viewer.get("websocket")

        if not viewer["conectado"] or websocket is None:

            continue

        lista_jogadores = []

        for player_id, jogador in jogadores_core.jogadores.items():

            item = {
                "id": player_id,
                "nome": jogador["nome"],
                "host": (player_id == jogadores_core.host_id),
                "conectado": jogador["conectado"],
            }

            lista_jogadores.append(item)

        estado = {
            "tipo": "estado_lobby",
            "jogadores": lista_jogadores,
            "sou_host": (viewer_id == jogadores_core.host_id),
            "codigo_recuperacao": viewer["codigo"],
            "jogos": JOGOS,
    ...
    for websocket in falhas:
        ...
        jogadores_core.desassociar_websocket(websocket)
...
async def websocket_lobby(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            if acao == "entrar":
                ...
                if jogo_selecionado is not None:
                    ...
                    await websocket.send_json(
                        {"tipo": "erro", "mensagem": ("Um jogo já está em andamento.")}
                    )
                ...
                if nome.lower() in nomes_existentes:
                    ...
                    await websocket.send_json(
                        {"tipo": "erro", "mensagem": ("Esse nome já está sendo usado.")}
                    )
                ...
                jogador = {
                    ...
                    "codigo": jogadores_core.gerar_codigo_recuperacao(),
                    "pontos": 0,
                    "conectado": True,
                    "websocket": websocket,
                }

                jogadores_core.jogadores[player_id] = jogador

                jogadores_core.associar_websocket(websocket, jogador)

                if jogadores_core.host_id is None:

                    jogadores_core.host_id = player_id

                print(f"[LOBBY] " f"{nome} entrou. " f"Código: " f"{jogador['codigo']}")

                await jogadores_core.enviar_sessao(websocket, jogador)
            ...
            # =================================================
                ...
                jogador = jogadores_core.jogador_por_token(token)
                ...
                await jogadores_core.fechar_conexao_antiga(jogador, websocket)

                jogadores_core.associar_websocket(websocket, jogador)

                if jogadores_core.host_id is None:

                    jogadores_core.host_id = jogador["id"]
            ...
            # =================================================
            ...
            elif acao == "recuperar_codigo":

                codigo = str(dados.get("codigo", "")).strip()

                jogador = jogadores_core.jogador_por_codigo(codigo)
                ...
                if not jogador:
                    ...
                    await websocket.send_json(
                        {
                            "tipo": "erro",
                            "mensagem": ("Código de recuperação " "não encontrado."),
                        }
                    )
            ...
            # =================================================
            ...
            elif acao == "selecionar_jogo":

                player_id = jogadores_core.websocket_para_jogador.get(websocket)

                # Somente HOST.

                if player_id != jogadores_core.host_id:

                    continue

                jogo_id = str(dados.get("jogo", "")).strip()

                jogo = obter_jogo(jogo_id)
                ...
                conectados = jogadores_core.ids_jogadores_conectados()
                ...
                if len(conectados) < 2:
                    ...
                    await websocket.send_json(
                        {
                            "tipo": "erro",
                            "mensagem": ("São necessários pelo menos " "2 jogadores."),
                        }
                    )
                ...
                print(f"[LOBBY] " f"Jogo selecionado: " f"{jogo_id}")
    ...
    # ========================================================
        ...
        player_id = jogadores_core.desassociar_websocket(websocket)
        ...
        if jogo_selecionado is None:

            era_host = player_id == jogadores_core.host_id
